...
class BlockState(AsteriskState):
    """ blocks a telephone number.
    """
    ...
    def __init__(self, call, dialed_extension):
        ...
        """
        super(BlockState, self).__init__(call)
        self.recording = None
    ...
    def enter(self):
        ...
        LOG.info('trying to block number, extension=%s', self.dialed_extension)

        matches = re.match(r'^[*]\d+[*](\d+)[#]$', self.dialed_extension)
        if matches:
            number = matches.group(1)
            phone_number = TelephoneNumber(number, validate=True)
            if phone_number.is_valid:
                LOG.info('blocking  phone number %s', phone_number)
                with Persistence(Config.TELEWALL_DATABASE_PATH) as p:
                    if not p.is_blocked(phone_number):
                        p.block(phone_number, source_name='extension')
                self.cleanup()
